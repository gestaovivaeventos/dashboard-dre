// EXECUTE the Terrazzo Omie cleanup (2022-2024). Deletes financial_entries
// scoped strictly by company_id = Terrazzo AND payment_date 2022-01-01..2024-12-31,
// then refreshes the DRE + cash-flow materializations for Terrazzo (and any
...
(async () => {
  // 1) Re-resolve Terrazzo, assert exactly one.
  ...
  if (!companies || companies.length !== 1) {
    throw new Error(`ABORT: esperava 1 empresa Terrazzo, achei ${companies?.length ?? 0}`);
  }
  const id = companies[0].id;
  log(`Terrazzo company_id = ${id}`);
  ...
  log(`PRE-DELETE: alvo(2022-2024)=${beforeTarget}  manter(>=2025)=${beforeKeep}  total=${beforeTotal}`);
  if (beforeTarget === 0) {
    log("Nada a remover (alvo=0). Encerrando sem delete.");
    return;
  }

  // 3) DELETE — strictly scoped: company_id = Terrazzo AND payment_date in [FROM, TO].
  ...
  // 5) Refresh materializações (DRE + fluxo de caixa) para Terrazzo + destinos roteados.
