...
function ValorFixoCategoryGroup({
  ...
  // Os controles da diretoria moram na LINHA DO CONTRATO (ver
  // `ControlesContrato`), não aqui: a aprovação é por contrato, porque uma
...
export function ValorFixoManager({ companyId, year }: { companyId: string; year: number }) {
  ...
  async function primeiroSetorComConteudo(ids: string[]): Promise<string | null> {
    ...
    for (const id of ids) {
      const res = await getValorFixoCategorias(companyId, year, id);
