...
//  - 'mensal': o valor vale de `mesInicio` até dezembro (serviço novo em julho
//    conta jul..dez);
...
export function serieItem(valor: number, mesInicio: number, periodicidade: Periodicidade): number[] {
...
type ItemSerie = { valorMensal: number; mesInicio: number; periodicidade: Periodicidade };
...
export function categoriaSerie(itens: ItemSerie[]): number[] {
  ...
  itens.forEach((it) => {
    const s = serieItem(it.valorMensal, it.mesInicio, it.periodicidade);
...
export function totalItem(valor: number, mesInicio: number, periodicidade: Periodicidade): number {
  return serieItem(valor, mesInicio, periodicidade).reduce((a, b) => a + b, 0);
}
