...
export function buildOnePageReportUserPrompt(input: OnePageInput): string {
  const header = [
    ...
    input.feat_contas_receber_aberto
      ? `Contas a receber em aberto Feat (Omie, saldo em aberto por titulo, departamentos selecionados, referencia ${input.feat_contas_receber_aberto.referencia}): total_em_aberto=${input.feat_contas_receber_aberto.total_em_aberto}, total_em_atraso=${input.feat_contas_receber_aberto.total_em_atraso} (${input.feat_contas_receber_aberto.percentual_em_atraso}% do aberto), titulos_em_aberto=${input.feat_contas_receber_aberto.titulos_em_aberto}, titulos_em_atraso=${input.feat_contas_receber_aberto.titulos_em_atraso}, clientes_em_aberto=${input.feat_contas_receber_aberto.clientes_em_aberto}, clientes_em_atraso=${input.feat_contas_receber_aberto.clientes_em_atraso}. Sao valores ainda NAO recebidos, nao receita realizada. Aging e ranking de clientes (maior atraso primeiro) no JSON, campo feat_contas_receber_aberto (aging[] e principais_clientes[])`
      : null,
