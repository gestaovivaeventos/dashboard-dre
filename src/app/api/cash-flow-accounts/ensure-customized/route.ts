...
// Lazy-forks the global Cash Flow plan into a per-company plan when an admin
// starts customizing the structure of a specific company. Idempotent: if the
// company already has a custom plan, this is a no-op.
export async function POST(request: Request) {
  ...
  const { data: forked, error: forkError } = await supabase.rpc(
    "cash_flow_accounts_fork_to_company",
