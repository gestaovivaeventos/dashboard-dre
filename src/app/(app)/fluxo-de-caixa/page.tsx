...
export default async function CashFlowPage({ searchParams, params }: CashFlowPageProps) {
  ...
  const [accDreResultado, accAmounts] = await Promise.all([
  ...
  const displayRows: CashFlowDisplayRow[] = cashFlowAccounts.map((account) => {
    ...
    const hasChildren = cashFlowAccounts.some((a) => a.parent_id === account.id);
