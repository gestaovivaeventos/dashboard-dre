-- =============================================================================
-- Roteamento: resolvedor de mapeamento MULTI-CAMADA para lancamentos roteados.
-- =============================================================================
-- Contexto:
--   20260602140000 fez o lancamento roteado usar SO o mapeamento do destino.
--   Mas os codigos de categoria da Omie podem diferir entre origem e destino,
--   e frequentemente a categoria do departamento roteado so esta mapeada na
--   ORIGEM (no DRE proprio dela). Usar so o destino deixava o lancamento sem
--   mapeamento -> descartado.
--
-- Correcao:
--   Resolvedor multi-camada por prioridade, para um lancamento roteado:
--     0) override roteado (routed_category_mapping)            [explicito]
--     1) mapeamento da empresa de DESTINO (efetiva)            [trata como destino]
--     2) mapeamento da empresa de ORIGEM                       [reusa o DRE da origem]
--     3) mapeamento GLOBAL (company_id IS NULL)
--   A conta resolvida e traduzida por CODIGO para o plano exibido
--   (scopeDreAccounts/translateToScopedId), entao cai na conta de mesmo codigo
--   no destino.
--
--   Para lancamento NAO roteado: destino = origem = company_id, entao as
--   camadas 1 e 2 colapsam e o comportamento fica IDENTICO ao historico
--   (empresa > global).
--
--   Quando NENHUMA camada mapeia a categoria (nem origem, nem destino, nem
--   global), o lancamento continua sem destino — nesse caso use a secao de
--   override na tela de mapeamento do destino para vincular manualmente.
-- =============================================================================
