...
- **Uma tabela só, em formato de extrato bancário**: `src/components/vb/statement-table.tsx` (`VbStatementTable`) desenha tanto o extrato do credor (cronológico, separador por ano com totais) quanto a revisão do lote (ordem de planilha, coluna do SALDO da planilha, flags e ações por linha) — as opções são props. A cor carrega o tipo (entrada verde, saída vermelha, rendimento azul), o saldo negativo é vermelho e a divergência acima da tolerância pinta a linha de âmbar. Não crie uma segunda tabela para o VB; a faixa de indicadores das duas telas também é compartilhada (`stat-strip.tsx`).
