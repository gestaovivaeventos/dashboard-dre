...
Nenhum contrato vai à ClickSign sem o aval do **aprovador único** (`CASE_CONTRACT_APPROVER_EMAIL` em `contract-config.ts`, hoje o Pedro — regra nominal registrada em `user-exceptions.ts`). Fluxo: `rascunho` → `enviarParaAprovacao` → `aguardando_aprovacao` (e-mail ao aprovador via Resend + contador no item Contratos do menu + faixa na lista) → `aprovarContrato` → `aguardando_assinatura` → `assinado` (webhook). `devolverContrato` volta para `rascunho` e manda o motivo por e-mail a quem pediu (`approval_requested_by`). Se o próprio aprovador clica em enviar, aprova e envia direto.

- **Ordem na ClickSign**: cliente + testemunha no grupo 1, contratado (o aprovador) no grupo 2 — ele assina por último. A aprovação **não** é o papel "approve" da ClickSign: a mesma pessoa em dois papéis no documento assina tudo de uma vez, o que quebraria a ordem.
