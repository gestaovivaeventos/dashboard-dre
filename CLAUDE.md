...
- **Importação é uma vez só**: não há item de menu; a Visão geral oferece "Importar histórico da planilha" enquanto não existe histórico aprovado (nem lote pendente), e a página `/vb/importar` recusa novo upload depois disso — a tela continua acessível por URL, como histórico das importações. Depois da aprovação, tudo é lançamento manual (`Novo lançamento`) na tela do credor.
- **Uma tabela só, em formato de extrato bancário**: `src/components/vb/statement-table.tsx` (`VbStatementTable`) desenha tanto o extrato do credor (separador por ano com totais) quanto a revisão do lote (coluna "Conf." com ✓/⚠ contra o SALDO da planilha, flags e ações por linha) — as opções são props. A cor carrega o tipo (entrada verde, saída vermelha, rendimento azul), o saldo negativo é vermelho e a divergência acima da tolerância pinta a linha de âmbar. Não crie uma segunda tabela para o VB; a faixa de indicadores das duas telas também é compartilhada (`stat-strip.tsx`). Extrato e revisão listam do mais recente para o mais antigo; o saldo de cada linha é calculado em ordem cronológica antes da inversão e antes de qualquer filtro (`VbStatementTable` inverte para exibir). A tela do credor continua server component e entrega os lançamentos ao `statement-view.tsx` (client), que monta faixa, gráfico da evolução do saldo, filtros (ano, tipo, busca) e a tabela.
