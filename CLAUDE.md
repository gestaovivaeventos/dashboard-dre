...
### Orçamento — entrevista do "Planejamento dos gestores" é base zero (`src/lib/orcamento/entrevista-prompt.ts`)

O prompt da entrevista com a IA (Etapa 2 do método) mora nesse módulo **puro e testado**, não no arquivo de actions (`"use server"` não pode exportar função sem virar endpoint, e foi por isso que o prompt antigo nunca teve teste). Decisão de 23/09/2026: a entrevista deixou de ser "gastou X em 2026, mantém em 2027?" e virou **orçamento base zero** (Pyhrr, 3G, Drucker): o gasto do ano anterior é informação, não direito adquirido.

- **Roteiro em 4 blocos**: contexto do ano (o que muda no setor), pacote de decisão por item (propósito, teste do zero "se não existisse, contrataria hoje?", alternativas, driver preço × volume, decisão), gastos novos com prioridade (essencial / importante / desejável) e **fechamento com desafio** (resumo do total contra o realizado + uma pergunta que confronta a proposta) antes do `[[FECHAR]]`.
- **A profundidade vem da materialidade** (`materialidade()`, Pareto): itens que somam 80% do total, qualquer item com ≥ 10%, e todos quando a base tem até 3 itens recebem `[pacote completo]`; a cauda recebe `[conferência rápida]` (mantém? para quê?). É a válvula contra entrevista interminável — não a remova para "aprofundar tudo".
- **A condução vem da classe da despesa** (`classeDespesa()`): estrutural (aluguel, impostos, pró-labore: só valor atualizado, sem teste do zero), contratual (pacote completo), discricionária (teste do zero é central), variável (driver e sazonalidade, sem vocabulário de contrato). Sem palavra conhecida no nome, a IA classifica sozinha na primeira mensagem. Para calibrar uma categoria, mexa na lista de palavras ou em `CATEGORIA_DESCRICOES` (regra por categoria prevalece sobre tudo).
- **O mês a mês do ano anterior entra no prompt** (`realizadoContexto`, com meses > 1,5× a média marcados como fora da curva). O cliente manda em `promptCtx.realizadoMeses`; sem ele o caminho rápido cai em 12 nulos e a IA só vê total e média.
- **A justificativa da proposta é o que a Diretoria lê na validação**: o rabo JSON pede 4 a 8 frases com contexto do ano, decisão por item relevante, novos com prioridade e a variação do total. O JSON dos itens **não mudou** (descrição, valor, mês, periodicidade, origem); campos por pacote (prioridade, impacto se cortar) são a fase seguinte, não construída.
