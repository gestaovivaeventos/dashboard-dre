...
- Fases seguintes previstas no modelo, não construídas: edição de lançamentos aprovados (hoje a única exclusão de aprovado é o Desvincular da triagem da Omie).
...
- `/api/cron/vb-monthly-reports` — `0 12 5 * *` (09:00 BRT, dia 5). Avisa os gestores do VB que os extratos do mês anterior estão disponíveis em `/vb/relatorios`; **não envia ao credor**.
