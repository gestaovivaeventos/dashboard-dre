...
**O contexto tem bloco próprio no relatório.** `contextoControladoria` (campo do `OnePageReportSchema`, `null` quando não há contexto) vira a faixa "Contexto do período", logo abaixo do Diagnóstico Principal, na tela, no PDF e no e-mail. Existe porque nenhum outro campo garante que o texto do CSC chegue ao leitor: `destaques` só aparece via bloco de alertas, que prioriza riscos e **corta em 3 cards** (`mapAlertas`), e `leituraPorIndicador`/`justificativa` não são renderizados — foi assim que a explicação do IPTU do Terrazzo sumiu mesmo com a IA tendo escrito. O bloco não passa por `enabledBlocks`: contexto escrito é contexto exibido, em qualquer empresa/template.
