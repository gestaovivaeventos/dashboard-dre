...
- **Novo lançamento é uma operação com várias linhas**: cabeçalho (data e descrição, iguais para todas) + linhas `credor · tipo · valor`, sem obrigar o líquido a fechar em zero (pagar vários credores no mesmo dia não fecha; transferência entre dois fecha). Período e taxa aparecem só quando há linha de rendimento e valem para todas elas ("juros do mês para todos"). As linhas vão num único INSERT (tudo ou nada) e compartilham `vb_entries.group_id` — o vínculo existe no banco, mas o extrato não o exibe. A montagem das linhas é a função pura `src/lib/vb/new-entries.ts` (`buildEntryRows`, com testes); a action `createVbEntries` só confere os credores e grava. Não existe mais lançamento de credor único: é o mesmo formulário com uma linha.
